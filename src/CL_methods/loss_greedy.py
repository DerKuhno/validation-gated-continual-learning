...
class Loss_Greedy(ReplayBuffer):
    """
    ...
    def __init__(self, old_replay_buffer: ReplayBuffer, model_ref):
        super(Loss_Greedy, self).__init__(
        ...
        self.prune_frac = self.config.method_params.prune_frac
    ...
    def choose_indices(self, error_values: torch.Tensor, indices_of_episode: List[Tuple[int, int]], k_keep: int, k_prune: int) -> List[Tuple[int, int]]:
        ...
        input:  
            error_values: tensor of error values for each index
            indices_of_episode: list of tuples containing the indices of the episode, same length as error_values
            k_keep: number of indices to keep
            k_prune: number of indices to prune based on highest error values (likely to be outliers/noise/close to anomalies)

        output:
            list of tuples containing the indices to keep after pruning
        """
    ...
    def add_data(self, total_new_indices):
        ...
        """
        logging.info("--- using replay strategy: loss greedy ---")
        ...
        # last episode is handled separately to prune high error values
        error_values = torch.zeros(len(total_new_indices), device=self.device)
        for start_batch in range(0, len(total_new_indices), self.config.val_batch_size):

            end_batch = min(start_batch+self.config.val_batch_size, len(total_new_indices))
            ...
            loss_values = self.model.criterion(self.model(mb_x), mb_y)
            error_values[start_batch:end_batch] = torch.mean(loss_values, dim=(1, 2))
        
        self.episode_indices[-1] = self.choose_indices(
            error_values, 
            self.episode_indices[-1], 
            k_keep=data_per_episodes,
            k_prune = int(math.floor(self.prune_frac * len(self.episode_indices[-1])))
        )

        for episode in range(total_episodes-1):
            error_values = torch.zeros(len(self.episode_indices[episode]), device=self.device)
