...
from .utils.convert_to_events import convert_time_series_to_events, NANOSECONDS_IN_SECOND
...
class ESAScores(Metric):
    ...
    def __init__(
        ...
        if select_labels is None or len(select_labels) == 0:
            ...
            select_labels = {
                col: np.atleast_1d(val) for col, val in select_labels.items()
            }
    ...
    def score(self, y_true: pd.DataFrame, y_pred: np.ndarray) -> dict:
        ...
        if self.full_range is None:  # automatic full range
            self.full_range = (
                min(y_true["StartTime"].min(), min(y_pred[..., 0])),
                max(y_true["EndTime"].max(), max(y_pred[..., 0])),
            )
        else:
            assert self.full_range[0] <= y_true["StartTime"].min(), print(f"y_true['StartTime'].min() > self.full_range[0]: {y_true["StartTime"].min()} > {self.full_range[0]}")
            assert self.full_range[1] >= y_true["EndTime"].max(), print(f"self.full_range[1] < y_true['EndTime'].max(): {self.full_range[1]} < {y_true["EndTime"].max()}")
            assert self.full_range[0] <= min(y_pred[..., 0]), print(f"y_true['StartTime'].min() > min(y_pred[..., 0]): {y_true["StartTime"].min()} > {min(y_pred[..., 0])}")
            assert self.full_range[1] >= max(y_pred[..., 0]), print(f"self.full_range[1] < max(y_pred[..., 0]): {self.full_range[1]} < {max(y_pred[..., 0])}")
        ...
        for aid in filtered_y_true["ID"].unique():
            ...
            for p, pred in enumerate(events_pred):
                ...
                if not any(intersections):
                    false_positive_events.append(pred)
                    continue
        ...
        result_dict = {
            "alarming_precision": precision_one_detection,
...
if __name__ == "__main__":
    ...
    y_true = [
        ...
        [
            "id_2",
            pd.to_datetime("2015-01-07"),
            pd.to_datetime("2015-01-08"),
            "Anomaly",
            "Multivariate",
            "Local",
            "Subsequence",
