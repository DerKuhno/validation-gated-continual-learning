...
def convert_time_series_to_events(
    vector=[
        ...
        [pd.to_datetime("2015-01-05"), 1],
        [pd.to_datetime("2015-01-10"), 1],
        [pd.to_datetime("2015-01-18"), 1],
        [pd.to_datetime("2015-01-20"), 0],
    ]
):
    """
    ...
    def find_runs(x):
        ...
        if n == 0:
            ...
            return np.stack(
                (run_starts[run_values > 0], run_ends[run_values > 0])
            ).transpose()
